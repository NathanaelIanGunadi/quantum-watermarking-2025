...
def _ensure_full_measure(qc):
    """Ensure the circuit has explicit measure ops on all qubits."""
    has_measure = any(inst.name == "measure" for inst, _, _ in qc.data)
    if has_measure:
        return qc
...
def transpile_for_backend(qc, backend, optimization_level: int = 1):
    """Transpile helper that mirrors the paper’s assumption (opt level = 1)."""
